...
$outputFile = __DIR__ . '/cars_fixed.csv';
...
while (($row = fgetcsv($input, 0, ';')) !== false) {
    ...
    if (count($row) >= 4) {
        $brand = trim($row[0]);
        $model = trim($row[1]);
        $yearFrom = (int)trim($row[2]);
        
        // Обработка поля year_to - заменяем "-" на null или текущий год
        $yearTo = trim($row[3]);
        if ($yearTo === '-' || empty($yearTo)) {
            $yearTo = date('Y'); // Текущий год вместо прочерка
        } else {
            $yearTo = (int)$yearTo;
        }
        ...
        ]) ? '1' : '0';  // Используем 1 и 0 вместо true/false
